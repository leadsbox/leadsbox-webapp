...
import { TrendingUp, TrendingDown, Users, MessageSquare, Clock, Target, BarChart3, PieChart, Download, RefreshCw } from 'lucide-react';
...
const EMPTY_ANALYTICS: Analytics = {
  ...
  trends: {
    ...
    responseTimesTrend: [],
    leadsByLabel: [
      { date: new Date().toISOString(), value: 45, label: 'NEW' },
      { date: new Date().toISOString(), value: 20, label: 'IN_PROGRESS' },
      { date: new Date().toISOString(), value: 15, label: 'FOLLOW_UP_REQUIRED' },
      { date: new Date().toISOString(), value: 10, label: 'WON' },
      { date: new Date().toISOString(), value: 5, label: 'LOST' },
    ],
  },
  performance: {
    topSources: [],
...
const AnalyticsPage: React.FC = () => {
  ...
  const fetchAnalytics = React.useCallback(
    async ({ background = false }: { background?: boolean } = {}) => {
      ...
      try {
        ...
        const message = (err as AxiosError<{ message?: string }>).response?.data?.message || 'Unable to load analytics overview.';
        setError(message);
        notify.error({
  ...
  const currentAnalytics = useMemo(() => analytics ?? EMPTY_ANALYTICS, [analytics]);
    ...
    <div className='p-4 sm:p-6 space-y-4 sm:space-y-6'>
      ...
      <div className='grid grid-cols-1 md:grid-cols-4 gap-3 sm:gap-4'>
        <Card>
          ...
          <CardContent>
            {loading ? <Skeleton className='h-8 w-16' /> : <div className='text-2xl font-bold'>{overview.totalLeads}</div>}
        ...
        <Card>
          ...
          <CardContent>
            {loading ? <Skeleton className='h-8 w-16' /> : <div className='text-2xl font-bold'>{overview.activeThreads}</div>}
            <div className='flex items-center space-x-1 text-xs'>
        ...
        <Card>
          ...
          <CardContent>
            {loading ? <Skeleton className='h-8 w-16' /> : <div className='text-2xl font-bold'>{formatPercentage(overview.conversionRate)}</div>}
            <div className='flex items-center space-x-1 text-xs'>
        ...
        <Card>
          ...
          <CardContent>
            {loading ? <Skeleton className='h-8 w-16' /> : <div className='text-2xl font-bold'>{overview.avgResponseTime}h</div>}
            <div className='flex items-center space-x-1 text-xs'>
                  ...
                  <div className='h-[240px]'>
                    <ResponsiveContainer width='100%' height='100%'>
                      <LineChart data={trends.leadsOverTime}>
                        ...
                        <Tooltip labelFormatter={(value) => new Date(value).toLocaleDateString()} formatter={(value: number) => [value, 'Leads']} />
                        <Line type='monotone' dataKey='value' stroke={colors.primary} strokeWidth={2} dot={{ r: 4 }} activeDot={{ r: 6 }} />
                      </LineChart>
                    </ResponsiveContainer>
                  </div>
                )}
                  ...
                  <div className='h-[240px]'>
                    <ResponsiveContainer width='100%' height='100%'>
                      <RechartsPieChart>
                        ...
                        >
                          {(trends.leadsByLabel ?? []).map((entry, index) => (
                  ...
                  <div className='h-[220px]'>
                    <ResponsiveContainer width='100%' height='100%'>
                      <LineChart data={trends.responseTimesTrend}>
                        ...
                        />
                        <Line type='monotone' dataKey='value' stroke={colors.secondary} strokeWidth={2} dot={{ r: 4 }} activeDot={{ r: 6 }} />
                      </LineChart>
                    </ResponsiveContainer>
                  </div>
                )}
                  ...
                  <div className='space-y-4'>
                    {performance.topSources.map((source, index) => (
                      <div key={source.source} className='flex items-center justify-between'>
                        ...
                        <Badge variant='secondary'>{formatPercentage(source.conversionRate)}</Badge>
                      </div>
                    ))}
                  </div>
                )}
                      ...
                      <div key={agent.agent.id} className='flex items-center justify-between'>
                        <div className='flex items-center space-x-3'>
                          <Avatar className='h-10 w-10'>
                            ...
                            <AvatarFallback>{agent.agent.name?.charAt(0)?.toUpperCase() || '?'}</AvatarFallback>
                          </Avatar>
                          <div>
