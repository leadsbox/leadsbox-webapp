...
const AutomationBuilder: React.FC<AutomationBuilderProps> = ({ initialFlow, onClose, onSave }) => {
  ...
  const updateFlow = useCallback(
    (updater: (current: AutomationFlow) => AutomationFlow, pushHistory = true) => {
      setFlow((current) => {
  ...
  const handleDragStart = (event: DragStartEvent) => {
    if ('clientX' in event.activatorEvent && 'clientY' in event.activatorEvent) {
      ...
      setLatestPointer({ x: e.clientX, y: e.clientY });
    }
  };
  ...
  const addNodeFromPalette = (type: AutomationNodeType, payload: Record<string, unknown>, position: { x: number; y: number }) => {
    updateFlow((current) => {
      ...
      const nodes = type === 'trigger' ? [...current.nodes.filter((existing) => existing.type !== 'trigger'), node] : [...current.nodes, node];
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    const data = active.data.current as
      | { source?: string; nodeId?: string; type?: AutomationNodeType; payload?: Record<string, unknown> }
      | undefined;
    ...
    if (data?.source === 'canvas-node') {
      ...
      if (Math.abs(dx) > 0 || Math.abs(dy) > 0) {
        updateFlow((current) => ({
          ...current,
          nodes: current.nodes.map((node) => (node.id === data.nodeId ? { ...node, x: snapToGrid(node.x + dx), y: snapToGrid(node.y + dy) } : node)),
        }));
      }
    }
  ...
  const handleNodeMoveEnd = useCallback(
    (nodeId: string, delta: { x: number; y: number }) => {
      ...
      updateFlow((current) => ({
        ...current,
        nodes: current.nodes.map((node) => (node.id === nodeId ? { ...node, x: snapToGrid(node.x + dx), y: snapToGrid(node.y + dy) } : node)),
      }));
    },
    [scale, updateFlow],
  );
  ...
  const handleStartConnection = useCallback(
    (nodeId: string, direction: 'input' | 'output', branch?: 'true' | 'false') => {
  ...
  const handleSaveDraft = () => {
    ...
    if (!result.ok) return;
    const saved = { ...flow, status: 'DRAFT' as const, updatedAt: new Date().toISOString() };
    persistFlow(saved);
    setTouched(false);
        ...
        <div className='flex flex-1 overflow-hidden'>
          ...
          <div className='relative flex-1 overflow-hidden'>
            ...
            <div className='absolute left-4 top-4 flex items-center gap-3 lg:hidden'>
              ...
              {selectedNodeId && <Badge variant='secondary'>Node: {selectedNodeId}</Badge>}
            </div>
          </div>
